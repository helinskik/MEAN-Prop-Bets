...
let DataService = class DataService {
    constructor(http) {
        this.http = http;
        this.betUrl = 'http://localhost:3000/api/bets';
        this.eventUrl = 'http://localhost:3000/api/events';
        this.gameUrl = 'http://localhost:3000/api/games';
        this.playerUrl = 'http://localhost:3000/api/players';
    ...
    getBets() {
        return this.http.get(this.betUrl, {
            ...
            .pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["map"])((bets) => {
    ...
    getEvents() {
        return this.http.get(this.eventUrl, {
            ...
            .pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["map"])((events) => {
    ...
    getGames() {
        return this.http.get(this.gameUrl, {
            ...
            .pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["map"])((games) => {
    ...
    getPlayers() {
        return this.http.get(this.playerUrl, {
            ...
            .pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["map"])((players) => {
...
let EntriesPageComponent = class EntriesPageComponent {
    ...
    selected($event) {
        ...
        this.dataService.getPlayers().subscribe((players) => {
            this.players = players.filter(item => item.gameId == this.gameId);
            this.tablePlayers = [];
...
let FormPageComponent = class FormPageComponent {
    ...
    radioChange(event) {
        ...
        if (!found) {
            ...
            let newBet = { _id: "0", question: question, questionText: questionText, guess: guess, answer: '', playerId: this.playerId, groupId: this.groupId, eventId: this.event._id };
    ...
    submit() {
        if (this.isAdmin) {
            this.guesses.forEach(guess => {
                ...
                this.dataService.updateBet(guess).subscribe((bet) => {
            ...
            this.guesses.forEach(guess => {
                ...
                this.dataService.insertBet(guess).subscribe((bet) => {
                    console.log("Player bet, ", bet);
                });
            });
            this.router.navigate(['entries']);
    ...
    ngOnInit() {
        ...
        this.dataService.getEvents().subscribe((events) => {
            if (events !== null && events.length > 1) {
                ...
                this.firstEvent = this.event = events.sort((a, b) => {
    ...
    submit() {
        ...
        if (this.form.valid) {
            if (this.gameText == "Create New Group" && this.form.value.group == "Submit Picks") {
                let data = { playerId: 0, gameId: 0, playerName: "", gameName: "Submit Picks" };
                this.dialogRef.close(data);
            }
            else if (this.gameText == "Create New Group") {
                let game = { _id: "", name: this.form.value.group, eventId: this.event._id };
                this.dataService.insertGame(game).subscribe((game) => {
                    console.log("Game created, ", game);
                    this.gameId = game._id;
                    this.gameText = game.name;
                    let player = { _id: "", name: this.form.value.name, gameId: this.gameId };
                    this.dataService.insertPlayer(player).subscribe((player) => {
                        console.log("Player created, ", player);
                        let data = { playerId: player._id, gameId: player.gameId, playerName: player.name, gameName: this.gameText };
                        this.dialogRef.close(data);
                    });
                });
            }
